...
static inline SurfaceResult intersect_glass_sphere(Vec o, Vec d, double t, Object obj){
    ...
    Vec first_o = add(o, mul(d, t)); // 交点を計算

    Vec normal = normalize(sub(first_o, s.o)); // 交点における法線
    
    double cos_1 = -dot(d, normal); // 入射角を計算
    double sin_1 = sqrt(1.0 - cos_1 * cos_1);

    double sin_2 = sin_1 * 1.0 / obj.refractive_index; // 屈折角を計算(スネルの法則)
    double cos_2 = sqrt(1.0 - sin_2 * sin_2);

    Vec first_d = add(mul(d, 1.0 / obj.refractive_index),
                      mul(normal, (1.0 / obj.refractive_index) * cos_1 - cos_2)); // 屈折ベクトルを計算
    first_d = normalize(first_d); // 正規化

    Vec first_o_offset = offset_pos(first_o, mul(normal, -1)); // 座標を下に浮かす

    Hitrecord hit = intersect_function(first_o_offset, first_d, obj, 0); // 球から出る屈折の交点求めるための関数を呼び出す
    ...
    Vec second_o = add(first_o_offset, mul(first_d, hit.t)); // 二回目の球から出る屈折の交点
    
    Vec exit_normal = normalize(sub(second_o, s.o)); // 交点における法線

    double cos_3 = dot(first_d, exit_normal); // 入射角を計算  
    double sin_3 = sqrt(1.0 - cos_3 * cos_3);

    double sin_4 = sin_3 * obj.refractive_index / 1.0; // 屈折角を計算(スネルの法則)
    ...
    // 第2項で exit_normal を足すことで、しっかり「外向き」のベクトルを作る
    Vec n_exit = mul(exit_normal, -1.0);
    Vec second_d = add(mul(first_d, n_ratio),
                   mul(n_exit, n_ratio * cos_3 - cos_4));
    second_d = normalize(second_d);

    res.next_o = offset_pos(second_o, exit_normal); // 座標を浮かす
    res.next_d = second_d ;// 次の屈折方向を作成
    res.color = (Vec){0, 0, 0};  // 方向を色で見る
    return res;
}
